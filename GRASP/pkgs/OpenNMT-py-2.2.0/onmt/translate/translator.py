...
class Translator(Inference):
    ...
    def _score_target(
        ...
        # batch_s = []
        # gs = torch.swapaxes(gold_scores, 0, 1).cpu().detach().numpy()
        # tgt = torch.swapaxes(gold, 0, 1).cpu().detach().numpy()
        # for (s,t) in zip(gs, tgt):
        #     for (_s, _t) in zip(s,t):
        #         batch_s.append(self._tgt_vocab.itos[int(_t)] + ' ' + str(_s))
        #     batch_s.append("$$")
        # with open('/home/yeminyu/work/retro/mt_result/check_likelihood/likelihood_rsmi_600000_id_fail.txt', 'a+') as f:
        #     for a in batch_s:
        #         f.write(a + '\n')
